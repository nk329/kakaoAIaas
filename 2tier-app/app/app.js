...
app.post('/api/users', async (req, res) => {
  try {
    ...
    res.status(201).json({ id: result.insertId, username, email, message: '사용자가 성공적으로 추가되었습니다.' });
  } catch (error) {
    console.error('Database error:', error);
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        .server-info { margin-top: 20px; padding: 10px; background-color: #e8f4f8; border-radius: 5px; }
        form { margin-top: 20px; padding: 15px; background-color: #f0f0f0; border-radius: 5px; }
    ...
    <body>
      <div class="container">
        ...
        <div id="userList" class="tabcontent"><h2>사용자 목록</h2><div id="users-table">데이터베이스에서 사용자 정보를 불러오는 중...</div></div>
